    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <a href="{{ route('users.index') }}" title="back" class="inline-flex items-center px-4 py-2 bg-gray-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-900 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150" ><img src="{{ asset('assets/back.svg') }}" alt="Back Icon" class="w-3 h-3"></a><br><br>
                ...
                <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <br>
                    <div>
                        <label for="profile_pic">Picture</label><br>
                        <input type="file" name="profile_pic" accept=".png,.svg,.jpg,.jpeg" class="block w-80 appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                    </div>
                    @error('profile_pic') <span class="text-red-500 error">{{ $message }}</span><br> @enderror 
                    <br>                   
                    <div>
